...
class VAE:
    ...
    def __init__(self,
                 input_shape,
                 conv_filters, #number of kernels
                 conv_kernels, #size of kernel
                 conv_strides,
                 latent_space_dim):
        self.input_shape = input_shape # [28, 28, 1]
        self.conv_filters = conv_filters # [2, 4, 8] : 1st layer has 2 filters, 2nd has 4 etc
        self.conv_kernels = conv_kernels # [3, 5, 3] : 1st layer has a kernel with square size (3,3) /
                                                        # 2nd layer has a kernel with square size (5,5)
    ...
    def _calculate_kl_loss(self, y_target, y_predicted): #𝐷𝐾𝐿=0.5∗(𝜎2+𝜇2−1−𝑙𝑜𝑔 𝜎2) [between standard normal
                                                            # distribution and another normal distribution]
    ...
    def _add_decoder_output(self, x):
        conv_transpose_layer = Conv2DTranspose(
            filters=1, #it depends on the channel size which is 1 in this case cause gray images
    ...
    def _build_encoder(self):
        ...
        self.encoder = Model(encoder_input, bottleneck, name="encoder") #used keras function (input,output, name)
    ...
    def _add_conv_layer(self, layer_index, x):
        ...
        x = conv_layer(x) # this means we getting the keras layer and we are applying it to our graph x
    ...
    def _add_bottleneck(self, x):
        ...
        x = Lambda(sample_point_from_normal_distribution,
                   name="encoder_output")([self.mu, self.log_variance]) #lambda is a layer used to wrap mathematical function
        return x


if __name__ == "__main__":
    autoencoder = VAE(
        input_shape= (48,1,1        ), #(28, 28, 1)
